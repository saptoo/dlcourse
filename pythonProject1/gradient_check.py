...
def check_gradient(f, x, delta=1e-6, tol=1e-4):
    '''
    ...
    Arguments:
      ...
      delta: step to compute num    erical gradient
    ...
    while not it.finished:
        ...
        x_plus_h_ix = x.copy()
        x_plus_h_ix[ix] += delta
        x_minus_h_ix = x.copy()
        x_minus_h_ix[ix] -= delta
        numeric_grad_at_ix = (f(x_plus_h_ix)[0] - f(x_minus_h_ix)[0]) / (2 * delta)
        # print(x_plus_h_ix)
        # print(str(f(x_plus_h_ix)[0]) + "\t" + str(f(x_minus_h_ix)[0]))
        # print(str(analytic_grad_at_ix)+ "\t" + str(numeric_grad_at_ix))
        # print("\n")
        # TODO compute value of numeric gradient of f to idx
        if not isclose(numeric_grad_at_ix, analytic_grad_at_ix, tol):
